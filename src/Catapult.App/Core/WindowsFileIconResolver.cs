...
public static class WindowsFileIconResolver
{
    public static Icon Resolve(string fullName)
    {
        if (".url".Equals(Path.GetExtension(fullName), StringComparison.InvariantCultureIgnoreCase))
        {
        ...
        var icon = ShellIcons.GetIcon(fullName);
