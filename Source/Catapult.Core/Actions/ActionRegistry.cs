...
public class ActionRegistry
{
    ...
    private readonly List<IIndexable> _actions = [];
    ...
    public ISearchFrame? GetSearchFrame(IIndexable[] indexables)
    {
        if (!indexables.Any())
        {
            var getUpdatedState = () => $"{_updateCounter}-{SearchResources.UpdateCounter}";
            var fetchIndexables = () => new IndexableResult(_indexers.SelectMany(x => x()).ToArray(), getUpdatedState());
        ...
        if (closedActionType != null && closedActionType == typeof(IAction<StringIndexable>))
        {
            ...
            {
                if (Activator.CreateInstance(type) is IAutocomplete autocomplete)
                {
        ...
        var targetTypes = Type.EmptyTypes;
